    ...
    <div className="w-full space-y-6 pb-20 sm:space-y-8 sm:pb-8">
      ...
      <section className="space-y-4">
        {primaryCourse ? (
          ...
          <div className="py-14 text-center rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex flex-col items-center gap-4">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-brand-primary/20 to-brand-secondary/20 flex items-center justify-center ring-4 ring-brand-primary/10">
              <Icon name="book-open" className="w-9 h-9 text-brand-primary" />
            </div>
            <div className="space-y-1">
              <p className="text-slate-800 dark:text-white font-bold text-lg">No courses started yet</p>
              <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Pick up where your journey begins</p>
            </div>
            <button onClick={() => navigateToFilteredCourses('all')} className="mt-1 inline-flex items-center gap-2 px-5 py-2.5 bg-brand-primary text-white text-sm font-bold rounded-xl hover:bg-brand-secondary transition-colors shadow-lg shadow-brand-primary/20">
              <Icon name="search" className="w-4 h-4" /> Find something new
            </button>
          </div>
        )}
      </section>
