...
const Toast: React.FC<{ message: string; isVisible: boolean; onClose: () => void; type?: 'success' | 'error' }> = ({ message, isVisible, onClose, type = 'success' }) => {
    ...
    return (
        <div className={`fixed bottom-6 left-0 right-0 px-4 flex justify-center z-[9999] transition-all duration-500 ease-out pointer-events-none ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            <div className={`glass-reflection pointer-events-auto flex items-center gap-3 px-6 py-3 rounded-full border shadow-2xl backdrop-blur-md ${isError ? 'bg-red-500/90 border-red-400 text-white' : 'bg-white/90 dark:bg-slate-800/90 border-white/40 dark:border-slate-700 text-slate-800 dark:text-white'}`}>
...
const CoursePreview: React.FC<CoursePreviewProps> = ({ course, onLoginClick, onBack, isDarkMode, setDarkMode, user, onProfileUpdate }) => {
    ...
    const onEnrollClick = async () => {
        await handleEnroll(course, appliedCoupon?.code ?? undefined, discountedPrice ?? undefined);
        if (appliedCoupon) {
