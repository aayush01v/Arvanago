...
const SidebarLayout: React.FC<SidebarLayoutProps> = ({
  ...
  const currentPage = useMemo(() => {
    ...
    const staticPages = new Map<string, { title: string; subtitle: string; trail: string[] }>([
      ...
      ['store', { title: 'Store', subtitle: 'Exclusive goods', trail: ['Store'] }],
    ]);
      ...
      >
        ...
        <div className="relative z-10 flex min-h-screen">
          ...
          >
            ...
            />
            <main className={`relative ${location.pathname.startsWith('/chat') ? 'z-40 p-0 sm:p-4 h-[calc(100dvh-var(--app-shell-header-height))] md:h-auto overflow-hidden' : location.pathname.startsWith('/store') ? 'z-10 p-0 sm:px-4' : 'z-10 px-2 pb-24 pt-4 sm:px-6 sm:pb-6 lg:px-10'}`}>
              {(location.pathname.startsWith('/chat') || location.pathname.startsWith('/store')) ? (
