...
const ChatPage: React.FC = () => {
    ...
    const [isStartingCall, setIsStartingCall] = useState(false); // Bug 2 Fix: loading state
    ...
    const startCall = async (type: 'video' | 'audio') => {
        ...
        // Bug 2 Fix: Don't open the modal until we have a valid callId.
        // The old "optimistic open with callId=null" pattern caused the modal's
        // useEffect to fire before createRoom completed, so no streams were set up.
        setIsStartingCall(true);
        setCurrentCallType(type);
        setIsCaller(true);

        try {
            // createRoom sets up the PC, gets media, creates SDP offer and Firestore doc
            const callId = await webrtcService.createRoom(currentUser.uid, activeChatUser.uid, type);

            // Only open the modal now that we have a real callId and streams are ready
                        ...
                        <div className="px-4 md:px-6 py-4 border-b border-white/10 flex items-center justify-between bg-white/40 dark:bg-slate-900/40 backdrop-blur-md sticky top-0 z-30">
                            ...
                            <div className="flex items-center gap-2 md:gap-4 flex-shrink-0 relative">
                                ...
                                <button
                                    ...
                                    className="p-3 md:p-3.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-brand-primary/10 hover:text-brand-primary dark:hover:bg-slate-700 transition-all text-slate-600 dark:text-slate-300 shadow-sm border border-transparent hover:border-brand-primary/20 hover:scale-105 active:scale-95 disabled:opacity-60 disabled:cursor-wait"
                                    title={isStartingCall ? 'Starting call...' : 'Start Call'}
                                >
                                    {isStartingCall
