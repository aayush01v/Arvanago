...
const Toast: React.FC<{ message: string; isVisible: boolean; onClose: () => void }> = ({ message, isVisible, onClose }) => {
  ...
  return (
    <div className={`fixed bottom-6 left-0 right-0 px-4 flex justify-center z-[9999] transition-all duration-500 ease-out pointer-events-none ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
      <Card variant="glass" className="glass-reflection pointer-events-auto flex items-center gap-3 rounded-full px-4 py-2 backdrop-blur-md text-slate-800 dark:text-white">
